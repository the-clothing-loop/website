...
func TestUserGetAllOfChainRoutePrivacyBasicCase(t *testing.T) {

	route_privacy := 1

	// logged user
	chain, user, token := mocks.MockChainAndUser(t, db, mocks.MockChainAndUserOptions{RoutePrivacy: &route_privacy, RouteOrderIndex: 3, OverrideLatitude: new(float64), OverrideLongitude: new(float64)})
