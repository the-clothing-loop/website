...
func TestLoginFlowToken(t *testing.T) {
	...
	app.AssertNotErrorNow(t, err, "CreateUnverifiedToken should return a nil err")
...
LIMIT 1
	...
	app.AssertNotErrorNow(t, res.Error, "Database call responded with an error")
	...
	_, err = auth.AuthenticateToken(db, token)
	assert.NotNilf(t, err, "Unverified token (%s) should not be useable", token)

	// verify token
	_, newToken, err := auth.OtpVerify(db, *user.Email, token)
	app.AssertNotErrorNow(t, err, "Token should pass verification (%s) %v", token, err)

	// ensure verified token is usable for authenticate
	_, err = auth.AuthenticateToken(db, newToken)
	app.AssertNotErrorNow(t, err, "Verified token should be useable (%s) %v", token, err)
